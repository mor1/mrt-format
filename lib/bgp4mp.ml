...
let parse subtype buf = 
  let get_ips buf = Afi.(function
    | IP4 -> 
        IPv4 (get_h4_peer_ip buf), IPv4 (get_h4_local_ip buf)
    | IP6 ->
        (IPv6 ((get_h6_peer_ip_hi buf), (get_h6_peer_ip_lo buf)), 
         IPv6 ((get_h6_local_ip_hi buf), (get_h6_local_ip_lo buf)))
  )
  in
  ...
  let pf hlen buf = 
    ...
    let header = match int_to_tc subtype with 
      | Some (MESSAGE|LOCAL|STATE) ->
          ...
          let peer_ip, local_ip = get_ips h afi in
